...
public class CategoryResource extends Resource{
    ...
    public Response searchCategories(@MatrixParam("Name") String name){
        CategoryJSON category = new CategoryJSON(0, name, null, null, null);   
    ...
    @Path("/{CatId: \\d+}")
    @Produces({"application/json"})
    public Response getCategory(@PathParam("CatId") int catId) throws JsonProcessingException{
        categoryExtendedInfoViewConfig();
    ...
    @Path("/{CatId: \\d+}")
    //@RolesAllowed({"User", "Admin"})
    public Response removeCategory(@PathParam("CatId") int catId){
...
*/
    ...
    protected void categoryPublicViewConfig(){
    ...
    protected void categoryExtendedInfoViewConfig(){
        SimpleBeanPropertyFilter extView = SimpleBeanPropertyFilter.serializeAll();
        Map<String, SimpleBeanPropertyFilter>  filters = getPublicViews();
        filters.put("CategoryPublicView", extView);
        FilterProvider provider = setFilters(filters);
        mapper.setFilterProvider(provider);
    }
}
